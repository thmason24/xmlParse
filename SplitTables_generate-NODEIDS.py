              ...
              #  sys.stderr.write(str(type(i)) + "\n")
                if "record-id" in i:
                    ...
                    if first:
                        ...
                        etree.SubElement(newNode,"node-id")                        
...
nodeIDs = []
for i in docTree.xpath("//document-tree-node"):
        nodeIDs.append(int(i.find("node-id").text))
nodeCount = max(nodeIDs)+1
print nodeCount
#add new nodes
for i in newNodeList:
    #add node ID and append
    i.xpath("node-id")[0].text = str(nodeCount)
    docTree.append(i)
    nodeCount += 1
